 ...
 * Verifies that associative mode follows capture group order from taxonomy regex_pattern
 */
...
class CorrectedAssociativeModeTest {
  ...
  testEdgeDirectionConsistency() {
    this.test('Edge Direction Consistency', () => {
      ...
      // Edge groups should follow regex_pattern: associative_{source}_to_{target}
      associativeEdges.forEach(edge => {
        this.assert(edge.group.includes('_to_'),
          `Edge group should follow regex_pattern: ${edge.group}`);
      });
